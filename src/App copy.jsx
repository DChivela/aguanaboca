...
// Gerar as seções a partir do menuData
let sections = [];
try {
  sections = generateSections(menuData);
} catch (error) {
...
function MenuItem({ item, onAddToCart, itemKey }) {
  ...
  // Detecta se o item é um dos mais pedidos
  const popularKeywords = ["hamburg", "calzone", "pizza"];
  const isPopular = popularKeywords.some((kw) =>
    (item.name || "").toLowerCase().includes(kw)
  );

  // Se o item tem preços por tamanho, inicializa o tamanho selecionado
  useEffect(() => {
  ...
  // Renderiza os botões de tamanho (P, M, G) quando aplicável
  ...
  // Preço que deve ser exibido (selecione size se existir)
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        {isPopular && (
          <span className="absolute top-2 left-2 bg-[#e63946] text-white text-xs px-2 py-1 rounded">Mais pedido</span>
        )}
      </div>
      ...
      <div className="flex-grow">
        ...
        <AnimatePresence>
          {isOpen && isPopular && (
            ...
            >
              <img src={item.image || "/default.jpg"} alt={item.name} className="w-full h-40 object-contain rounded-md mb-2" />
      ...
      {/* Modal "Outros" para itens não-populares */}
      <AnimatePresence>
        {isModalOpen && !isPopular && (
          <motion.div
          ...
          >
            ...
            >
              <div className="flex justify-between items-start">
                <h4 className="text-lg font-bold text-[#774936]">Outros</h4>
                <Button size="sm" onClick={toggleModal} className="bg-[#774936]">Fechar</Button>
              </div>
              <div className="mt-4">
                <img src={item.image || "/default.jpg"} alt={item.name} className="w-full h-44 object-contain rounded-md mb-2" />
...
// Componente para cada seção (categoria) com efeito gaveta
function MenuSection({ title, items, id, onAddToCart }) {
  return (
    <section id={id} className="mb-12">
      <div className="mb-4 bg-gradient-to-r from-[#e63946] to-[#f9c74f] p-2 rounded-lg">
